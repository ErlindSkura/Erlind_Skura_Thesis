...
def table_leakage(m: dict) -> str | None:
    ...
    difference is the amount by which a naive protocol would have overstated
...
def fig_qualitative(m: dict, names=("Z6-1", "Z2-2")) -> None:
    ...
    fig, axes = plt.subplots(len(names), cols, figsize=(3.1 * cols, 2.4 * len(names)))
    ...
    for row, name in enumerate(names):
        ...
        for col, (title, masks, colour) in enumerate(panels):
            ...
            ax.set_title(f"{title}\n{len(masks)} beads", fontsize=8)
            ax.axis("off")
        axes[row, 0].text(0.02, 0.03, f"{name} — {rec.magnification}×",
                          transform=axes[row, 0].transAxes, fontsize=8,
                          color="white",
                          bbox=dict(facecolor="black", alpha=0.6, pad=2))
    fig.suptitle("Held-out micrographs: each panel is scored by a model that "
                 "never saw this specimen", fontsize=9)
    fig.tight_layout()
...
def run() -> None:
    ...
    parts = [
        ...
        table_config(m), table_folds(m), table_maskrcnn(m), table_comparison(m),
        table_magnification(m), table_physical(m),
    ]
    leakage = table_leakage(m)
    if leakage:
        parts.append(leakage)

    out = RESULTS / "chapter5_tables.tex"
    out.write_text("\n\n".join(parts) + "\n", encoding="utf-8")
    print(f"wrote {out}")
