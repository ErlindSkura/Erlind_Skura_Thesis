...
changes: i) ingest, reading the LabelMe JSON files, recovering specimen
and magnification and extracting the pixel calibration from the instrument
banner; ii) prepare, cropping the banner, rasterising the polygons into
instance masks and emitting a COCO-format annotation file;
iii) partition, assigning images to the four leave-one-specimen-out
folds; iv) train and predict, fitting each method on each fold's training
partition and predicting on its held-out specimen; and v) evaluate and
