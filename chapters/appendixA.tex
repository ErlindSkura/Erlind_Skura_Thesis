...
This appendix lists the parts of the implementation in which a design decision is
...
rasterises only afterwards, so that overlapping beads remain separable. The
reflection convention below is the one described in
Section~\ref{sec:challenges}, where vertices are continuous coordinates and a
reflection is therefore $x \mapsto W - x$.
...
The following is the check that found the one-pixel error described in
Section~\ref{sec:challenges}. It rasterises the transformed polygons and compares
them against the directly transformed raster mask, and the two agree to an
intersection over union of 0.9995 under the correct convention and of only 0.87
under the incorrect one.
...
152 annotated beads and several hundred predictions, so that evaluating every
