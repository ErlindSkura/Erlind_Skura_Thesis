...
The dataset consists of SEM micrographs of electrospun nanofibre mats, acquired on
a Zeiss instrument at an accelerating voltage of 15.00\,kV and at working
distances between 8.5 and 11.0\,mm. Every micrograph is $1024 \times 768$ pixels,
8-bit greyscale, stored as JPEG.

The 11 micrographs are not independent samples. They derive from four specimens,
denoted Z2, Z4, Z5 and Z6, each imaged at $500\times$, $1000\times$ and
$3000\times$. The numeric suffix in a filename encodes the magnification rather
than a repeat acquisition, so Z2-1, Z2-2 and Z2-3 are three views of one specimen.
One combination is absent, as specimen Z4 has no $500\times$ micrograph, and
specimen Z6 uses the identifier Z6-4 for its $3000\times$ view. The four specimens
...
\begin{enumerate}
    ...
    through it. In these micrographs this is the common case rather than the
    exception, which is why no distinction is drawn between a free particle and a
    thickened fibre segment.

    \item \textbf{A cluster is one object.} Where several small particles lie
    together, the group is outlined once rather than resolved into its members.
\end{enumerate}
...
crystals present. What is counted here is therefore particle sites, and the merge
...
presented at very different resolutions, as the median bead of specimen Z6
...
Every micrograph carries a burned-in information banner occupying the bottom 32
...
statistical: thirteen copies of one micrograph contain the information of one
micrograph, so reporting 143 images, or 7\,878 bead instances, would misstate the
...
transformed versions of the same source micrograph in both the training and the
...
population of beads, so a partition placing Z2-1 in training and Z2-3 in testing
...
\begin{figure}[htbp]
    ...
    magnification, and the shaded cells are the 11 available micrographs. Specimen
    Z4 has no $500\times$ micrograph.}
    \label{fig:split_protocol}
\end{figure}

The protocol adopted is therefore \textbf{leave-one-specimen-out
cross-validation}. Each of the four folds trains on all micrographs of three
specimens and tests on all micrographs of the fourth, so every micrograph is used
...
which the 11 micrographs are split at random without regard to specimen. The
...
baseline requires no training data, so it is evaluated on all 11 micrographs
...
pyramid backbone, the anchor sizes, the detection budget, the optimiser, the
...
cells in low-contrast micrographs by Uka et al.\ \cite{uka2020faster}.
...
three departures from its defaults and the same gradient budget, while differing
...
default 640, since downscaling a $1024 \times 736$ micrograph by 0.625 would take
the median particle at $500\times$ from 14.3 to 8.9 pixels. Mosaic augmentation is
disabled, because composing four images and rescaling roughly halves the apparent
object size while 82.8\% of the particles are already below the small-object
threshold of COCO. The detection cap is raised to 400 for the same reason as for
the region-based models.
...
models draw on demand, so both YOLO models train on whole micrographs at 1024
...
network sees a micrograph changes materially what it can learn, for images
...
about eighteen grey levels separating a particle from the surrounding mat, but
...
fibre network with structure at the same spatial scale as the particles. The
...
\begin{itemize}
    ...
    accuracy as buy it, and a variant that loses to this row is a finding.
    ...
    particle-to-mat difference.

    \item \textbf{Background subtraction.} The particles are darker than the mat,
    so the illumination estimate is a grey closing, which erases dark features
    while preserving the slowly varying field. Its footprint has to exceed the
    largest object present, and since the largest annotated particle spans 214
    pixels the footprint is set to 257.
...
counts are pooled over micrographs before the ratio is formed, so a frame holding
...
\textbf{Areal density.} Beads per square millimetre, the form in which the
measurement is used for process comparison. For a micrograph of $W \times H$
