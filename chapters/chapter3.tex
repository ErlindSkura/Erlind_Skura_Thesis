...
A partition that does this is said to \emph{leak}. Leakage is the presence, in
the data a model is trained on, of information about the data it will be tested
on. It does not require the same image to appear twice: a second view of the same
specimen, or a rotated copy of the same image, carries much of the same
information, and a model that has learned those particular beads will score well
on the test partition without having generalised to anything. The score is then a
measurement of memory rather than of transfer, and it is optimistic by an amount
nobody can estimate after the fact. This is why the partition is defined over
specimens below, and why Section~\ref{sec:leakage} goes on to measure what the
naive alternative actually costs on this dataset.
...
pixels while the region-based models train on $512 \times 512$ crops. One
optimiser step therefore covers a whole $1024 \times 736$ image for a YOLO model
against one $512 \times 512$ crop for a region-based one, which is 2.9 times the
area, and the wall-clock times reported in
Section~\ref{sec:runtime} measure two different amounts of work per step rather
than two speeds. They are reported because they are what the campaign cost, and
they should not be read as a comparison of efficiency between the two families.
